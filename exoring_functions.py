...
def integrate2d(func, bounds, sigma):
    ...
    while True:
        ...
        if iteration == 1:
            ...
            if old_total_integral == 0 or abs((new_total_integral - old_total_integral) / old_total_integral) < 0.01 or old_total_integral < 1e-10:
...
class Animation:
    ...
    def generate_animation(self):
        ...
        def init():
            ...
            axs2.set_xlim(-(self.star.distance + self.star.radius + self.planet.radius), self.star.distance + self.star.radius + self.planet.radius)
            axs2.set_ylim(-(self.star.distance + self.star.radius + self.planet.radius), self.star.distance + self.star.radius + self.planet.radius)
            axs2.set_zlim(-(self.star.distance + self.star.radius + self.planet.radius), self.star.distance + self.star.radius + self.planet.radius)
        ...
        def update(frame):
            ...
            star_x_coords, star_y_coords, star_z_coords = self.generate_sphere_coords([0, 0, 0], sphere_radius=self.star.radius,
                                                                                 sampling_num=100)
            x_coords, y_coords, z_coords = self.generate_sphere_coords(centre + orbital_centre, sphere_radius=self.planet.radius,
                                                                  sampling_num=100)
            axs2.plot_surface(
