...
class SingleScatteringLaw:
    ...
    def __call__(self, alpha):
        scattering_angle = np.pi - np.abs(alpha)
        return (self.albedo / self.norm) * self.func(scattering_angle)
...
class Rayleigh(SingleScatteringLaw):
    ...
    def rayleigh_func(self, scattering_angle):
        return (1 + np.cos(scattering_angle) ** 2)
...
class HG(SingleScatteringLaw):
    ...
    def hg_func(self, scattering_angle):
        return (2 * (1 - self.g ** 2)) / (1 + self.g ** 2 - 2 * self.g * np.cos(scattering_angle)) ** (1.5)
...
class Mie(SingleScatteringLaw):
    ...
    def mie_func(self, scattering_angle):
        S1, S2 = msc.MieS1S2(self.m, self.X, np.cos(scattering_angle))
...
class Jupiter(SingleScatteringLaw):
    ...
    def hg_func(self, g, scattering_angle):
        return (2 * (1 - g ** 2)) / (1 + g ** 2 - 2 * g * np.cos(scattering_angle)) ** (1.5)

    def jupiter_func(self, scattering_angle):
        return self.f * self.hg_func(self.g1, scattering_angle) + (1 - self.f) * self.hg_func(self.g2, scattering_angle)
