...
def silicate_phase_func_plot(ax):
    ...
    ax.plot(np.cos(thetas), sc_atmos(thetas), label = 'Rayleigh scattering')
    ax.plot(np.cos(thetas), sc_dust(thetas), label = 'Mie scattering, averaged across size dist.')
    ax.plot(np.cos(thetas), sc_mie(thetas), label = 'Mie scattering, individual particle')
